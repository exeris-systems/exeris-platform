...
3. **Tailwind via `@exeris-systems/ui-kit`** (0.4.0 direction). Component library inherits the kit; don't ship a competing design system inside Studio. Note the wiring depends on the Tailwind major: the kit ships a JS preset for v3 and a CSS `@theme` entry for v4, and the frontend is on v4 — so it is the `@theme` import, not `presets: [...]`, which v4 removed.
...
The nearest such change is widening `exeris/domainDescribe` past `fields` / `actions` / `artefacts` — 0.4.0's entity detail view needs `relationships`. The projection already holds the full `DomainMetadata`, so the work is a cast; the ADR is not about which fields to add but about **how the shape distinguishes "there are none" from "this pipeline does not carry them"**, because `projections` and `eventHandlers` are reserved upstream and would arrive permanently empty. The argument is written out at `domainDescribe` in `ExerisProtocolExtensions.java`; take it from there rather than re-deriving it.
