...
3. **Tailwind via the SDK's UI kit** (0.4.0 direction). Component library inherits the kit; don't ship a competing design system inside Studio. Two names, and they are not interchangeable: `exeris-sdk-ui-kit` is the **directory** in the `exeris-sdk` repo, `@exeris-systems/ui-kit` is the **npm package** you install (published to GitHub Packages, so a scoped-registry `.npmrc` and a `read:packages` token are needed). Wiring depends on the Tailwind major: the kit ships a JS preset for v3 and a CSS `@theme` entry for v4, and the frontend is on v4 — so it is the `@theme` import, not `presets: [...]`, which v4 removed.
4. **A Studio surface is fed by the LSP if and only if it is about someone else's domain.** The user's entities, actions and relationships change while Studio is running and do not exist when we build, so the model-editing surfaces read them live over `exeris/*` — that is not a preference, it is the only thing that can work. Studio's screens over the *platform's own* domain (`Workspace` today, control-plane state later) are known at our build time and are generated by `@exeris/codegen-ts` from the `exeris-metadata` corpus. This is the same test as strong default 2, one layer up: **whose shape the surface describes**. Two consequences worth stating, because both are easy to get wrong: a generated screen must never become the source of truth for a *user* domain (that is the parallel metamodel in frontend clothing), and generated and hand-written screens must sit on the same `@exeris-systems/ui-kit` or the two halves drift apart visually and behaviourally.
5. **LSP transports**: stdio for IDE plugins, WebSocket for Studio frontend. Both speak the same JSON-RPC. Don't fork the method surface per transport.
6. **Sibling-repo orchestration** is currently in-job (clone + `mvn install` each upstream) per the 0.2.0 CI plan. A SNAPSHOT registry is a future option, not a near-term commitment.
7. **The standalone launcher is a consumer contract, and `LauncherIT` is what makes it one.** `exeris-platform-lsp` attaches a shaded `-standalone` jar; `LauncherIT` starts *that jar* as a separate `java -jar` process and drives a real LSP session through it. Do not delete it, do not move it to surefire (it needs `package` to have run), and do not "fix" a red build by skipping it. It exists because the neighbouring `exeris-kernel-diagnostics-cli` published a 0.11.0 shaded jar that initialised and then died on its first call — a build that never runs the artifact it ships cannot see that class of bug. Shading is also where the SDK's Jackson-3 polymorphic `MutationOp` / `MutationResult` vocabulary is most likely to break, and the IT covers exactly that path.
8. **JDK floor is 25** (`maven.compiler.release` in the root POM), matching `exeris-kernel`, `exeris-sdk` and `exeris-tooling` — all three are release 25, and `exeris-platform-bom` is where the exact upstream versions live, so read them there rather than trusting a number written here. This is a downstream constraint, not a style preference: `exeris-ai-bridge` runs our launcher beside `exeris-kernel-diagnostics-cli`, and emitting class-file 70 here would give a consumer two different JDK floors. Raise it only when the kernel does.
9. **`mvn deploy` goes to GitHub Packages, not Maven Central**, and Central is not close. It needs signing, sources/javadoc jars and a readiness gate this repo does not have (copy `exeris-kernel`'s `release` profile when the time comes), but the binding constraint is sequencing, not machinery: the rest of the stack has to be on Central first — `exeris-kernel` 0.12.0 (still in development), `exeris-sdk` 0.12.0, `exeris-tooling` 0.9.0 — and this repo reaches 0.6.0 at the earliest. Do not treat "add a `release` profile" as a ready task. When it is wired, **Central rides the same tag** — one cut, both registries — and stays disabled until then. The two channels differ in reversibility, not in trigger: a Packages coordinate can be republished, a Central version is spent forever, which is why only Central needs the signing and readiness machinery above. Today a publish is `.github/workflows/publish.yml`, and **a release there is a tag and nothing else** — pushing `v<x.y.z>` is what deploys; `workflow_dispatch` is a dry run that touches nothing remote. Not on push to `main`, deliberately: a whole development line shares one `-SNAPSHOT` coordinate, so publishing per merge produces artifacts that share a coordinate and differ in content. Trunk therefore sits on `<next>-SNAPSHOT` and `main` never carries a release version; the cut procedure is in `ROADMAP.md`.
...
- **ADR-084** (WebSocket Provider SPI) — the transport the Studio ↔ LSP connection will use. This repo is its *named consumer*: the six requirements it is built on are ours, and it reverses RFC-2026-06-18's SSE-only ruling because LSP is bidirectional and SSE is not. It ships **`preview`** at kernel 0.12 by decision, so anything bound to it is pinning a contract declared to move. Do not solve a missing transport with a third-party container — that was tried in PR #24 and closed.
...
The nearest such change is widening `exeris/domainDescribe` past `fields` / `actions` / `artefacts` — 0.4.0's entity detail view needs `relationships`. Two things about it are easy to get wrong. It is an **amendment to ADR-025**, not a new ADR: `docs/adr/ADR-025.link.md` prescribes exactly that for any change to this surface. And the "none vs not carried" distinction it has to preserve is **already** the canonical model's — `DomainMetadata` is `@JsonInclude(NON_NULL)`, so null is omitted and `[]` means none — so our job is not to invent a convention but to avoid flattening one, since `projections` and `eventHandlers` are reserved upstream and arrive unpopulated. The full argument is at `domainDescribe` in `ExerisProtocolExtensions.java`; take it from there rather than re-deriving it.
