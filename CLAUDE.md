...
4. **LSP transports**: stdio for IDE plugins, WebSocket for Studio frontend. Both speak the same JSON-RPC. Don't fork the method surface per transport — one `ExerisLanguageServer` implements it and `LspMain --websocket` only changes how bytes arrive. Three things about the WebSocket path are load-bearing and easy to undo by accident: **one server instance per session** (a shared one would race the index and break `MutationApplyService`'s documented single-reader-thread assumption), **`exit` closes the session, not the process** (one process serves every Studio tab — that is why the exit action is injected rather than `System.exit` inline), and **the bind address defaults to loopback** because the socket carries `exeris/applyMutation` and LSP has no authentication, so the interface is the only access control there is. `WebSocketLauncherIT` covers all three against the shaded jar.
